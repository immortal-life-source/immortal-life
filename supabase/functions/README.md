...
`sync-intelligence` runs PubMed and Europe PMC literature ingestion, ClinicalTrials.gov registry ingestion, Crossref retraction checks, and official regulatory feeds from EMA, SÚKL, FDA MedWatch, MHRA, Health Canada, and Australia’s TGA. Every candidate passes the same relevance scoring, duplicate clustering, freshness checks, and automatic quarantine rules before it can appear publicly. `member-intelligence` provides the signed-session Personal Longevity Radar for topics, organisations, countries, individual trials, in-app change alerts, and private briefings. `generate-briefings` is called by the database scheduler every Monday and idempotently generates one briefing per opted-in member and period. `record-utility-event` stores only bounded daily aggregate product events; it receives no visitor identifier.
