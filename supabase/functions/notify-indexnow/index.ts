...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [research, trials, regulatory, integrity, briefings] = await Promise.all([
      supabase.from('research_items').select('id').gte('last_seen_at', since).limit(1500),
      supabase.from('clinical_trials').select('id').gte('last_seen_at', since).limit(1500),
      supabase.from('regulatory_events').select('id').gte('last_seen_at', since).limit(500),
      supabase.from('research_integrity_events').select('id').gte('detected_at', since).limit(500),
    ...
    if (!urlList.length) return jsonResponse(req, { ok: true, submitted: 0 }, 200, 'POST')
    const result = await fetch('https://api.indexnow.org/indexnow', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ host: 'immortal.life', key: INDEXNOW_KEY, keyLocation: `${SITE}/${INDEXNOW_KEY}.txt`, urlList }) })
