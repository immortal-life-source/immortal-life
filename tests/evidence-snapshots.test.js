...
test('ISRCTN and DOAJ are explicit, commercially-cleared live feeds', async () => {
  ...
  assert.match(sync, /RUN_TIME_BUDGET_MS = 15_000/)
  assert.match(sync, /requestedSource === 'all' \? RUN_TIME_BUDGET_MS : 10_000/)
  assert.match(sync, /SCHEDULED_INGESTION_PAUSED/)
